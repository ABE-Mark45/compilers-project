...
TEST(ParseTableGeneratorTest, firstTest) {
  ...
  map<basic_string<char>,
      vector<pair<basic_string<char>,
                  vector<pair<basic_string<char>, bool>>>>>::iterator itr;
  for (itr = first.begin(); itr != first.end(); itr++) {
    map<basic_string<char>,
        vector<pair<basic_string<char>,
                    vector<pair<basic_string<char>, bool>>>>>::iterator itr2;
...
TEST(ParseTableGeneratorTest, followTest) {
  ...
  map<string, vector<string>> expected_follow = {{"E", {"$", ")"}},
                                                 {"E_dash", {"$", ")"}},
                                                 {"F", {"*", "+", "$", ")"}},
                                                 {"T", {"+", "$", ")"}},
                                                 {"T_dash", {}}};
  ...
  for (itr = follow.begin(); itr != follow.end(); itr++) {
    ...
    for (itr2 = expected_follow.begin(); itr2 != expected_follow.end();
         itr2++) {
      if (itr->first == itr2->first) {
        //ASSERT_EQ(itr->first,itr2->first);
        ASSERT_EQ(itr->second, itr2->second);
...
TEST(ParseTableGeneratorTest, tableTest) {
  ...
  map<pair<string /*NT*/, string /*token*/>,
      ...
      expectedTable = {
          ...
          {{"F", "id"}, {{"id", true}}}};
  map<pair<string /*NT*/, string /*token*/>,
      vector<pair<string /*NT or Terminal*/, bool>>>::iterator itr;
  ASSERT_EQ(table.size(), expectedTable.size());
  for (itr = table.begin(); itr != table.end(); itr++) {
    map<pair<string /*NT*/, string /*token*/>,
        vector<pair<string /*NT or Terminal*/, bool>>>::iterator itr2;
    bool found = false;
